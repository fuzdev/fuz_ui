...
- `DialogContent` takes a `pane` prop (default `true`) to toggle the fuz_css `.pane` card class; pass `pane={false}` for a chromeless surface (also set `Dialog`'s `content_selector`, since click-outside keys on `.pane`)

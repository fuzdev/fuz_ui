import"../chunks/DsnmJJEf.js";import{f as a,b as r,d as i,r as o}from"../chunks/BjUGcBtz.js";import{M as s}from"../chunks/Cmurzb_d.js";const d=`# mdz Formal Grammar Specification
...
- \`__\` → literal text "\\_\\_" (empty italic)
...
- \`snake**_case_**\` → snake***case*** (bold containing italic)
...
_AbsolutePath_ = \`/\` _PathChar_+
_RelativePath_ = (\`./\` | \`../\`) _PathChar_+
...
- \`'internal'\` - absolute (\`/path\`) or relative (\`./path\`, \`../path\`)

**Auto-detected URLs:**

Must start with \`https://\` or \`http://\`. Scheme matching is case-insensitive (RFC 3986 §3.1) — \`HTTPS://\`, \`Https://\`, etc. all match. The original casing is preserved in the emitted reference; browsers normalize schemes themselves.
Must be preceded by a non-word character (not \`A-Z\`, \`a-z\`, or \`0-9\`) or start of string. \`xhttps://...\` is plain text, not a link.
...
Absolute paths start with \`/\`. Relative paths start with \`./\` or \`../\`.
All path types must be preceded by whitespace or start of string.
Continues until whitespace or trailing punctuation.
Trailing punctuation handling same as auto-URLs.

**Rendering note:**

Absolute paths are resolved through SvelteKit's \`resolve()\` to apply the base path. Relative paths are resolved against the \`base\` prop/context when provided, producing absolute paths passed through \`resolve()\`. Without \`base\`, relative paths use raw hrefs, letting the browser resolve them against the current URL.

**Safe-reference filter:**

At render time, link references containing \`:\` are required to start with \`http://\` or \`https://\`. Other schemes (\`javascript:\`, \`data:\`, \`file:\`, etc.) produce no \`href\` — children render as plain text. See \`mdz_is_safe_reference\`.
...
\`\`\`
_Heading_ = <column 0> _HeadingMarker_ SPACE _HeadingContent_ <followed by newline or EOF>
...
Must be followed by a newline or end of file.
...
Each heading gets a lowercase slugified \`id\` derived from its text content, enabling fragment links. Inline formatting is stripped to extract plain text, then \`slugify\` produces the id (e.g. \`## Async Patterns\` → \`id="async-patterns"\`, \`## AsyncStatus\` → \`id="asyncstatus"\`).
...
\`\`\`
_HorizontalRule_ = <column 0> \`---\` SPACE* <followed by newline or EOF>
\`\`\`
...
\`\`\`
_Codeblock_ = <column 0> _OpenFence_ NEWLINE _CodeContent_ _CloseFence_ <followed by newline or EOF>
...
Components and elements must be registered via \`MdzRoot\` (which sets \`mdz_components_context\` and \`mdz_elements_context\`).
...
interface MdzBaseNode {
	type: string;
	start: number;
	end: number;
}

interface MdzTextNode extends MdzBaseNode {
	type: 'Text';
	content: string;
}

interface MdzCodeNode extends MdzBaseNode {
	type: 'Code';
	content: string;
}

interface MdzCodeblockNode extends MdzBaseNode {
	type: 'Codeblock';
	lang: string | null;
	content: string;
}

interface MdzBoldNode extends MdzBaseNode {
	type: 'Bold';
	children: Array<MdzNode>;
}

interface MdzItalicNode extends MdzBaseNode {
	type: 'Italic';
	children: Array<MdzNode>;
}

interface MdzStrikethroughNode extends MdzBaseNode {
	type: 'Strikethrough';
	children: Array<MdzNode>;
}

interface MdzLinkNode extends MdzBaseNode {
	type: 'Link';
	reference: string;
	children: Array<MdzNode>;
...
interface MdzParagraphNode extends MdzBaseNode {
	type: 'Paragraph';
	children: Array<MdzNode>;
}

interface MdzHrNode extends MdzBaseNode {
	type: 'Hr';
}

interface MdzHeadingNode extends MdzBaseNode {
	type: 'Heading';
	level: 1 | 2 | 3 | 4 | 5 | 6;
...
interface MdzElementNode extends MdzBaseNode {
	type: 'Element';
	name: string;
	children: Array<MdzNode>;
}

interface MdzComponentNode extends MdzBaseNode {
	type: 'Component';
	name: string;
	children: Array<MdzNode>;
}
...
- Heading nodes have \`level\` (1-6), \`id\` (lowercase slugified text for fragment links), and \`children\` (inline content)
- Link nodes have \`reference\` (URL/path), \`link_type\` ('external' | 'internal'), and \`children\` (display content). Internal covers both absolute (\`/path\`) and relative (\`./path\`, \`../path\`) references
- Component and Element nodes have \`name\` and \`children\`
- Bold, Italic, Strikethrough, and Paragraph nodes have \`children\` arrays
`;var l=a('<div class="mt_xl5"><!></div>');function _(e){var n=l(),t=i(n);s(t,{get content(){return d}}),o(n),r(e,n)}export{_ as component};

import"../chunks/DsnmJJEf.js";import{f as a,b as i,d as r,r as s}from"../chunks/HfSnbY9A.js";import{M as o}from"../chunks/C5ugl2hd.js";const l=`# mdz Specification

⚠️ AI-generated and unverified

This document provides a comprehensive guide to mdz syntax, features, and usage patterns.

For a high-level introduction, see /docs/mdz. For formal grammar rules, see ../grammar. For the streaming model, see ../streaming.

---

## Design Philosophy

mdz prioritizes _predictability_ with one canonical pattern per feature, preferring false negatives over false positives to minimize surprise.
...
- auto-detected external URLs (\`https://\`, \`http://\`)
- auto-detected absolute paths (\`/path\`)
- auto-detected relative paths (\`./path\`, \`../path\`)
...
Paths starting with \`/\`, \`./\`, or \`../\` are automatically linked:

\`\`\`
See /docs/api for the API documentation.
Visit /docs/mdz/grammar for the formal grammar.
See ./sibling for a sibling file.
See ../shared/utils for a parent-relative path.
\`\`\`

Absolute paths (\`/docs/api\`) are resolved through SvelteKit's \`resolve()\` to apply the base path. Relative paths (\`./foo\`, \`../bar\`) are resolved against the \`base\` prop/context when provided, producing absolute paths passed through \`resolve()\`. Without \`base\`, relative paths use raw hrefs, letting the browser resolve them against the current URL.

### Unsafe URI schemes

For security, \`MdzNodeView\` and \`MdzStreamNodeView\` strip the \`href\` from any link whose reference is not a \`http://\` or \`https://\` URL, a path (\`/\`, \`./\`, \`../\`), a fragment (\`#\`), or a query (\`?\`). \`javascript:\`, \`data:\`, \`file:\`, and other schemes render as their display text only — no clickable anchor is emitted.

\`\`\`
[click](javascript:alert(1))
\`\`\`

Renders as the literal text \`[click](javascript:alert(1))\` (the parser also rejects \`javascript:\` from auto-link detection, so this is defense in depth at both parse and render). The check lives in \`mdz_is_safe_reference\`.
...
- Each heading gets a lowercase slugified \`id\` for fragment links (e.g. \`## Async Patterns\` → \`id="async-patterns"\`)
...
- Closing fence followed by newline or EOF
...
\`\`\`svelte
<MdzRoot elements={new Map([['aside', true], ['code', true], ['marquee', true]])}>
...
\`\`\`svelte
<MdzRoot components={new Map([['Alert', Alert], ['Card', Card]])}>
...
| Block indent         | Up to 3 spaces allowed                | Must be column 0 (all block elements)                       |
| Heading no space     | \`#text\` valid                         | Invalid (space required)                                    |
| Setext headings      | \`text\\n---\` = \`<h2>\`                  | NOT supported (\`---\` always = HR)                           |
| Relative path links  | Not auto-linked                       | \`./path\` and \`../path\` auto-link                            |

**Rationale:**

One canonical syntax per feature reduces cognitive load, and stricter inline formatting rules minimize false positives for nontechnical users. mdz has no setext headings, so \`---\` after a paragraph always creates a horizontal rule (unlike CommonMark where it becomes an \`<h2>\` setext heading).
...
**File-structured static content** - Relative path auto-linking (\`./sibling.md\`, \`../shared/utils\`) makes mdz natural for wikis, indexes, and documentation trees where files reference their neighbors.
...
- Links (markdown, auto-detected URLs, absolute and relative paths)
...
`;var d=a('<div class="mt_xl5"><!></div>');function m(e){var n=d(),t=r(n);o(t,{get content(){return l}}),s(n),i(e,n)}export{m as component};

...
describe('library_gen_generate_ts', () => {
	...
	test('generates valid TypeScript with correct structure', () => {
		...
		// Check imports - only LibraryJson type now
		assert.ok(result.includes('import type {LibraryJson}'));

		// Check exports library_json (pre-parsed at gen time)
		assert.ok(result.includes('export const library_json: LibraryJson = {'));

		// Verify valid TypeScript structure (no syntax errors obvious)
		assert.ok(!result.includes('undefined'));
	});

	test('properly serializes library_json data', () => {
		...
		assert.ok(result.includes('"repo_name"'));
	});
	...
	test('properly serializes source_json with modules', () => {
		...
		// Verify source_json is embedded in library_json
		assert.ok(result.includes('"source_json"'));
	...
	test('handles empty modules array', () => {
		...
		assert.ok(result.includes('export const library_json'));
		assert.ok(result.includes('"modules": []'));
	});
